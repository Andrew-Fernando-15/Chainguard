    ...
    <div>
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {TECH_STACK.map((t, i) => (
            ...
            >
              ...
              <ul className="mt-3 space-y-1.5 text-sm text-frost/60">
                {t.items.map((item) => (
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-3">
          {SECURITY.map((s) => (
            <span key={s} className="glass flex items-center gap-2 rounded-full px-4 py-2 text-sm text-frost/70">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2">
          <SdgCard number="09" title="Industry, Innovation & Infrastructure" desc="Applying blockchain and AI infrastructure to modernize public-sector evidence handling." color="from-cyan/20 to-cyan/5" />
          <SdgCard number="16" title="Peace, Justice & Strong Institutions" desc="Strengthening trust in the justice system through tamper-proof, transparent evidence records." color="from-blue/20 to-blue/5" />
        </div>
      </section>
      ...
      <section id="team" className="mx-auto max-w-7xl px-5 py-20 lg:px-8">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {teamMembers.map((m, i) => (
            ...
            >
              <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-blue/30 to-cyan/30 text-lg font-semibold">
                {m.name.split(' ').map((n) => n[0]).join('')}
              </div>
              <h4 className="mt-4 font-semibold">{m.name}</h4>
              <p className="text-sm text-frost/50">{m.role}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-3xl px-5 py-20 lg:px-8">
        ...
        <form onSubmit={handleContact} className="glass mt-10 space-y-4 rounded-2xl p-8">
          <div className="grid gap-4 sm:grid-cols-2">
            <input required placeholder="Your name" className="rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-sm outline-none focus:border-cyan/50" />
            <input required type="email" placeholder="Email address" className="rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-sm outline-none focus:border-cyan/50" />
          </div>
          <textarea required rows={4} placeholder="Your message" className="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-sm outline-none focus:border-cyan/50" />
          <button type="submit" className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue to-cyan px-6 py-2.5 text-sm font-medium text-navy">
            <FiSend /> Send Message
          </button>
        </form>
      </section>
...
function SdgCard({ number, title, desc, color }) {
  return (
    ...
    >
      <span className="font-mono text-4xl font-bold text-frost/20">SDG {number}</span>
      <h4 className="mt-3 font-semibold">{title}</h4>
      <p className="mt-2 text-sm text-frost/55">{desc}</p>
    </motion.div>
  );
}
